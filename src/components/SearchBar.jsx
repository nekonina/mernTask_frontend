...
const SearchBar = () => {
    ...
    const {modal, handleSearch, proyectos} = useProyect()
    ...
    return (
        <Transition.Root show={modal.search} as={Fragment} afterLeave={() => setBusqueda('')  }>
                        ...
                        <Combobox.Options static className="max-h-72 scroll-py-2 overflow-y-auto py-2 text-sm text-gray-800">
                            {
                                proyectosFiltrados.map( proyecto => (
                                    <Combobox.Option
                                        key={proyecto._id}
                                        value={proyecto}
                                        className={({active}) => classNames('py-2 px-4 select-none cursor-default', active && 'bg-pink-600 text-white')}
                                    >
                                        {proyecto.nombre}
                                    </Combobox.Option>
                                ))
